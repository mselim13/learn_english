...
class _AiConversationPageState extends State<AiConversationPage> {
  static const _welcomeMessage =
      "Hi! I'm your English practice partner powered by Gemini AI.\n"
      "Send me a message in English (or Turkish) and let's start chatting!";
  ...
  bool _sending = false;
  String? _ttsBusyId;
  String _activeBackend = 'gemini';

  @override
  void initState() {
    super.initState();
    _turns = [const _ChatTurn(isUser: false, text: _welcomeMessage)];
    _initSpeech();
    _initTts();
  }
  ...
  Future<void> _sendUserText(String raw) async {
    ...
    try {
      ...
      // Gemini başarısız olduysa kullanıcıya sebebi bildir
      if (result.backend == 'local' && result.failReason != null) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Gemini: ${result.failReason} — offline şablona geçildi'),
            behavior: SnackBarBehavior.floating,
  ...
  void _clearChat() {
    setState(() {
      _turns
        ..clear()
        ..add(const _ChatTurn(isUser: false, text: _welcomeMessage));
    });
    _scrollToBottom();
  }
